...
namespace NE {
    ...
    void SpriteDataHandler(uint8_t *pData, uint32_t *pBmpStart, int pixelOffset, int pixelLen, int y, bool &copyline,
                           uint16_t *m_Palette16, uint32_t *m_Palette32) {
    ...
    EFileType check_file_type(char *data, size_t size) {
        // SP
        if (*(uint16_t *) data == 'PS') { return eFILE_TYPE_SPRITE; }

        if (*(uint32_t *) data == 'FFIR') {
            if (*(uint32_t *) (data + 8) == 'EVAW') // Wav
                return eFILE_TYPE_WAV;
            if (*(uint32_t *) (data + 8) == 'NOCA') // ani
                return eFILE_TYPE_ACON;
        }
        // Oggs
        if (*(uint32_t *) data == 'SggO') { return eFILE_TYPE_OGGS; }
        // tga
        const char *tgaFlag = "TRUEVISION-XFILE.";
        int off = size - strlen(tgaFlag) - 1;
        if (strncmp(data + off, tgaFlag, strlen(tgaFlag)) == 0) { return eFILE_TYPE_TGA; }
        // PNG
        if (*(uint32_t *) data & 0xFFFFFF == 'GNP') { return eFILE_TYPE_PNG; }
        // JPEG
        if (*(uint32_t *) data == 0xE0FFD8FF) { return eFILE_TYPE_JPEG; }
        // BMP
        if (*(uint16_t *) data == 'MB') { return eFILE_TYPE_BMP; }
        // TEXT
        if (strlen(data) == size) { return eFILE_TYPE_TEXT; }
        // Mp3
        Map3FrameHeader *header = (Map3FrameHeader *) data;
        if (header->sync1 == 0xff && header->sync2 == 0x7) {
            return eFILE_TYPE_MP3;
        }
        // fsb4
        if (*(uint32_t *) data == '4BSF') { return eFILE_TYPE_FSB4; }
        // rar
        if (*(uint32_t *) data == '!raR') { return eFILE_TYPE_RAR; }
    ...
    void Sprite::SaveImage(const char *filename, int index) {
        ...
        uint8_t *img_data = new uint8_t[sq.Width * sq.Height * 4]{0};
        for (int row = 0; row < sq.Height; row++) {
            for (int col = 0; col < sq.Width; col++) {
                ...
                img_data[datai * 4] = (uint8_t) ((pix & 0xff0000) >> 16);
                img_data[datai * 4 + 1] = (uint8_t) ((pix & 0xff00) >> 8);
                img_data[datai * 4 + 2] = (uint8_t) (pix & 0xff);
                img_data[datai * 4 + 3] = 0xff;
            }
        }
        UtilsSaveImageFile(filename, sq.Width, sq.Height, 32, (char *) img_data);
        delete[] img_data;
    }
    ...
    void WAS::Decode(std::string path) {
        for (int i = 0; i < mFrameIndecies.size(); i++) {
            ...
            if (frameHeader.Height >= (1 << 15) || frameHeader.Width >= (1 << 15) || frameHeader.Height < 0 ||
                frameHeader.Width < 0) {
            ...
            for (int j = 0; j < fHeight; j++) {
                ...
                SpriteOriginDataHandler((uint8_t *) (m_FileData.data() + offset), fWidth, &image_idx[image_offset],
                                        &image_alpha[image_offset]);
            }
            bool copyLine = true;
            ...
            std::string outpath = path + std::to_string(i) + ".x.bmp";
    ...
    Sprite *WDF::LoadSpriteHeader(uint32_t id, std::vector<PalSchemePart> *patMatrix) {
        ...
        if (header.Len > 12) {
            ...
            sprite.Times.resize(addonHeadLen);
            MEM_READ_WITH_OFF(wasReadOff, sprite.Times.data(), wasMemData, addonHeadLen);
        }
    ...
    bool WDF::LoadSpriteData(Sprite *sprite, std::vector<PalSchemePart> *patMatrix) {
        ...
        for (int i = 0; i < frameTotalSize; i++) {
            ...
            if (wasFrameHeader.Height >= (1 << 15) || wasFrameHeader.Width >= (1 << 15) || wasFrameHeader.Height < 0 ||
                wasFrameHeader.Width < 0) {
    ...
    Sprite *WDF::UnpackSprite(uint32_t id, std::vector<PalSchemePart> pal) {
        ...
        for (int i = 0; i < frameTotalSize; i++) {
            ...
            for (int j = 0; j < fHeight; j++) {
                ...
                int pixelLen = fWidth;
                SpriteDataHandler((uint8_t *) (data + offset), pBmpStart, 0, pixelLen, j, copyLine, m_Palette16,
                                  m_Palette32);
            }
    ...
    EFileType WDF::GetWasType(uint32_t id) {
        if (mIdToPos.find(id) == mIdToPos.end()) return eFILE_TYPE_UNKNOWN;
        Index index = mIndencies[mIdToPos[id]];
        char *data = (char *) (m_FileData.data() + index.offset);
        return check_file_type(data, index.size);
    }
